...
SINGLETON_IMPLENTATION(XWGeoTool);
...
- (void)parserDidStartDocument:(NSXMLParser *)parser{
    _list = [NSMutableArray arrayWithCapacity:80];
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(nullable NSString *)namespaceURI qualifiedName:(nullable NSString *)qName attributes:(NSDictionary<NSString *, NSString *> *)attributeDict{
    ...
    if ([elementName isEqualToString:@"CountryRegion"]){
        ...
        geoCountryRegion.code = [attributeDict objectForKey:@"Code"];
    ...
    if ([elementName isEqualToString:@"State"]) {
        if (self.list.lastObject) {
            ...
            state.code = [attributeDict objectForKey:@"Code"];
    ...
    if ([elementName isEqualToString:@"City"]) {
        if (self.list.lastObject) {
            if (self.list.lastObject.stateArr.lastObject) {
                ...
                city.code = [attributeDict objectForKey:@"Code"];
...
- (void)parserDidEndDocument:(NSXMLParser *)parser{
    ...
    if ([self.delegate respondsToSelector:@selector(parserDidEndGeoList:)]) {
        [self.delegate parserDidEndGeoList:self.list];
    }
}
